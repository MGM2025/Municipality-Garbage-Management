...
const HistoryPage = () => {
    ...
    const fetchPrevRequests = async () => {
        const response = await fetch(`${backendURL}/reqCollection/getAllReqCollections?userId=${userId}`, {
            method: 'GET',
            headers: {
        ...
        if(response.ok) {
            setPrevRequests(data.reqCollection);
        }
    }
    ...
    return (
        <>
            ...
            <div className="reqHistoryContainer min-h-[calc(100vh-4rem)] flex flex-col justify-between items-center px-6 relative">
